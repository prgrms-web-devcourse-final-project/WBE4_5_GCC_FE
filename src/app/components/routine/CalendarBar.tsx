...
export default function CalendarBar({
  ...
  return (
    <div className="mb-[20px] w-full max-w-md rounded-[10px] bg-white p-4 select-none dark:bg-[var(--dark-bg-primary)] dark:text-[var(--dark-gray-700)]">
        ...
        {thisWeek.map((day) => (
          ...
          >
            ...
            <div
              ...
              className={`flex h-[30px] w-[30px] cursor-pointer items-center justify-center rounded-full text-sm transition-colors hover:shadow-sm ${day.isToday && !day.isSelected ? 'border-2 border-[#ffb84c] bg-white text-black dark:bg-[var(--dark-bg-tertiary)] dark:text-[var(--dark-white)]' : ''} ${day.isSelected ? 'bg-[#ffb84c] text-white dark:text-[var(--dark-bg-primary)]' : ''} ${!day.isToday && !day.isSelected ? 'bg-[#EEF0F2] text-black dark:bg-[var(--dark-bg-tertiary)] dark:text-[var(--dark-white)]' : ''}`}
            >
              {day.date}
            </div>
          </div>
